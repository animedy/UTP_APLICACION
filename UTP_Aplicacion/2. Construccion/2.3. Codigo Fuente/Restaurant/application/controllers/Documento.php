...
class Documento extends CI_Controller 
{
	...
	**/
	function ListarPedidos()
	{
		...
		$data['listapedidosdetalle'] = $this->Model_Caja->getPedidoDetalle();
	...
	**/
	...
	{
		//lista los documentos que an sido pagados
		$data['DcumentoPagados'] = $this->Model_Caja->getListarCanceladoDocumento();
	...
	**/
	function ListaDocumentosAnulados()
	{
		...
		$data['DocumentoAnulados'] = $this->Model_Caja->DocumentosAnulados();
	...
	function VerDocumento($id = NULL)
	{
		if ($id != NULL) 
		{
			...
			foreach ($docu as $documento)  
			{
				...
				$idboleta  = $this->Model_Documento->insertBoleta($nombre,$dni,$fecha,$hora,$pedido,$estado,$empleado);
				$nro=0;
					...
					if($documento->idPedidos == $detalle->Pedidos_idPedidos) 
						{
                    		...
                    		$this->load->model('Model_DetalleDocumento');
                    		$this->Model_DetalleDocumento->insertBoletaDetalle($nro,$producto,$cantidad,$precio,$idboleta);
                		}
            	}
                ...
                $this->load->model('Model_Documento');
                $this->Model_Documento->updateBoleta($idboleta,$total);
                $emitido="emitido";
                $this->Model_Caja->updatePedido($pedido,$emitido);
            }
