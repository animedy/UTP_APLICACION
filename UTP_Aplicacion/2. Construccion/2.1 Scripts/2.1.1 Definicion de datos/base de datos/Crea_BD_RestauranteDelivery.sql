...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_motos`  AS  select `m`.`idPlaca` AS `Placa`,`m`.`Marca_Moto` AS `Moto`,`m`.`Soat` AS `Soat`,`e`.`Nombres` AS `Nombres`,`e`.`Apellidos` AS `Apellidos` from (`moto` `m` join `empleados` `e` on((`m`.`empleados_idEmpleados` = `e`.`idEmpleados`))) ;
